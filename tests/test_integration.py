...
from birkenbihl.models.translation import Translation, Language, TranslationType, TranslationResult
...
class TestCompleteTranslationFlow:
    ...
    async def test_complete_birkenbihl_workflow_lo_que_parecia(
        ...
        result = await mock_translation_provider.translate_birkenbihl(
        ...
        stored_translations = db_session.exec(
            select(Translation).where(Translation.source_text == original_text)
        ).all()
        ...
        dekodierung = next(
            t for t in stored_translations 
            if t.translation_type == TranslationType.WORD_FOR_WORD
        )
...
class TestDatabaseStorageBothTypes:
    ...
    def test_store_natural_and_dekodierung_for_same_text(self, db_session):
        ...
        all_translations = db_session.exec(
            select(Translation).where(Translation.source_text == source_text)
        ).all()
...
class TestBirkenbihIFormattingEndToEnd:
    ...
    def test_formatting_algorithm_with_test_data(self, mock_translation_provider, birkenbihl_test_data):
        ...
        for spanish_text in spanish_texts:
            ...
            assert all(
                word in lines[1] for word in german_word.split()
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_formatting_with_database_round_trip(
        self, mock_translation_provider, db_session
    ):
        ...
        assert "¿Cómo estás?" in lines[0] or all(
            word.replace('¿', '').replace('?', '') in lines[0] 
            for word in original_text.split()
        )
...
class TestRealWorldBirkenbihIExamples:
    ...
    @pytest.mark.asyncio
    async def test_specification_example_lo_que_parecia_formatting(
        self, mock_translation_provider
    ):
    ...
    @pytest.mark.asyncio
    async def test_specification_example_tenlo_por_seguro_alignment(
        self, mock_translation_provider
    ):
    ...
    @pytest.mark.asyncio
    async def test_specification_example_fueron_tantos_longer_sentences(
        self, mock_translation_provider
    ):
