...
def _has_field_value(language: Language, to_compare: str) -> bool:
    """
    Check if the language model has one its field values equal to the value_to_compare

    Returns True if it does, False otherwise
    """
    for field in fields(language):
        field_value = getattr(language, field.name, None)
        if field_value is None or field_value != to_compare:
            continue
        return True
    return False


def is_auto_detect(name_or_code: str) -> bool:
    """
    Returns True if the code is "auto-detect, False otherwise.
    """
...
def get_language_by(name_or_code: str) -> Language:
    ...
    Args:
        name_or_code: English, German name or ISO 639-1 language code

    Returns:
        Language object

    Raises:
        ValueError: If no language with the given name or code is found
    """
    if is_auto_detect(name_or_code):
        return AUTO_DETECT
    if name_or_code in SUPPORTED_LANGUAGES:
        return SUPPORTED_LANGUAGES[name_or_code]
    for language in SUPPORTED_LANGUAGES.values():
        if not _has_field_value(language, name_or_code):
            continue
        return language

    raise RuntimeError(f"No language contains value {name_or_code}")


def get_default_source_language() -> Language:
...
def get_default_target_language() -> Language:
    """Get the default source language

    Returns:
        Language object
    """
    return get_language_by("de")
