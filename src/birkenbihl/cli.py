...
from birkenbihl.models.requests import AutoDetectRequest, TranslationRequest
...
def _execute_translation(
    ...
    if source:
        ...
        request = TranslationRequest(text=text, source_lang=source_language, target_lang=target_language, title=title)
        translation = service.translate(request)
        return service.save_translation(translation)

    request = AutoDetectRequest(text=text, target_lang=target_language, title=title)
    translation = service.auto_detect_and_translate(request)
    return service.save_translation(translation)
