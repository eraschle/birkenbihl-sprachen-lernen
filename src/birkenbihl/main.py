...
class DatabaseService:
    ...
    def get_languages(self) -> Sequence[Language]:
        """Get all active languages."""
        with self.get_session() as session:
            return session.exec(select(Language).where(Language.is_active)).all()
    ...
    def get_translation_history(self, limit: int = 50) -> Sequence[Translation]:
        """Get translation history ordered by creation date."""
        with self.get_session() as session:
            return session.exec(select(Translation).order_by(Translation.created_at.desc()).limit(limit)).all()

    def search_translations(self, search_text: str, limit: int = 20) -> Sequence[Translation]:
    ...
    def get_translations_by_language_pair(
        self, source_lang_id: int, target_lang_id: int, limit: int = 20
...
class BirkenbihApp:
    ...
    def __init__(self) -> None:
        ...
        self.translation_provider = PydanticAITranslationProvider(
            model=model_string, api_key=self.config.ai_provider.api_key
        )
        ...
        with ui.column().style("max-width: 1200px; margin: 0 auto; padding: 20px"):
            ...
            with ui.card().style("width: 100%; margin-bottom: 20px"):
                ...
                self.text_input = ui.textarea(
                    placeholder="Text hier eingeben zum Übersetzen...", value="Lo que parecía no importante"
    ...
    def _format_alignment(self, original: str, word_for_word: str) -> str:
        ...
        # Simple alignment
        max_len = (
            max(len(w) for w in original_words + word_for_word_words) if original_words and word_for_word_words else 10
        )
    ...
    def _display_results(self, result: TranslationResult) -> None:
        ...
        with self.results_container:
            ...
            with ui.card().style("width: 100%; margin-bottom: 20px"):
                ...
                ui.label(result.natural_translation).style(
                    "font-size: 1rem; padding: 10px; background-color: #e3f2fd; border-radius: 5px"
                )
